...
; VERSION INTEGRADA: PERSONA 1 + PERSONA 2 + PERSONA 3
;
; PERSONA 1 -> Interfaz, entrada de usuario, pantallas
; PERSONA 2 -> Logica del juego (comparar, victoria, puntaje, turnos)
; PERSONA 3 -> Archivos, aleatoriedad, entrada oculta PvP
;====================================================================
...
;====================================================================
; SECCION DE DATOS GLOBALES (compartidas por los tres modulos)
;====================================================================

; ---- Variables de juego (Persona 2) ----
...
; ---- Variables de archivo (Persona 3) ----
...
; ---- Variables de interfaz (Persona 1) ----
...
MSG_OPCIONES   DB 0DH, 0AH
               DB '[R] Rendirse  [X] Reset  [S] Salir', 0DH, 0AH, '$'
...
MSG_SE_RINDIO  DB 0DH, 0AH, 'La palabra era: $'
...
MAIN:
    ...
    ;------------------------------------------------------------------
    ; LOOP DE RONDAS
    ; Persona 2 controla el flujo, Persona 1 muestra, Persona 3 carga
    ;------------------------------------------------------------------
RONDA:
    CALL IMPRIMIR_TURNO             ; P1: muestra "Turno del Jugador X"
    CALL CARGAR_PALABRA             ; P3: carga palabra (archivo o PvP)

LOOP_TURNO:
    CALL CALCULAR_PUNTAJE           ; P2: puntaje potencial segun errores
    CALL MOSTRAR_JUEGO              ; P1: dibuja el tablero completo

    CALL LEER_INPUT_JUGADOR         ; P1: lee letra o accion especial

    ; Verificar acciones especiales (Persona 1)
    ...
    ; Letra normal: procesarla (Persona 2)
    MOV  LETRA_ACTUAL, AL

    CALL VALIDAR_REPETIDA           ; P2: verifica si ya fue usada
    JC   LETRA_YA_USADA

    CALL COMPARAR_LETRA             ; P2: busca la letra en la palabra

    CALL DETECTAR_VICTORIA          ; P2: verifica si ya se adivino
    JC   GANO_RONDA

    CALL DETECTAR_DERROTA           ; P2: verifica si se agotaron intentos
    JC   PERDIO_RONDA
...
LETRA_YA_USADA:
    ...
    ;------------------------------------------------------------------
    ; Jugador se rindio (accion especial 1, Persona 1)
    ;------------------------------------------------------------------
ACCION_RENDIRSE:
    CALL MOSTRAR_RENDICION          ; P1: muestra la palabra y mensaje
    CALL PUNTAJE_DERROTA            ; P2: puntaje = 0
    CALL ACUMULAR_PUNTAJE
    JMP  FIN_RONDA

    ;------------------------------------------------------------------
    ; Reset de la ronda (accion especial 2, Persona 1)
    ;------------------------------------------------------------------
ACCION_RESET:
    ; Recargar la misma ronda desde el principio
    MOV  ERRORES, 0
    ...
    ;------------------------------------------------------------------
    ; Salir del juego (accion especial 3, Persona 1)
    ;------------------------------------------------------------------
ACCION_SALIR_TOTAL:
    MOV  AH, 4CH
    INT  21H
    ...
    ;------------------------------------------------------------------
GANO_RONDA:
    CALL MOSTRAR_JUEGO              ; P1: tablero final con la palabra
    LEA  DX, MSG_VICTORIA
    CALL IMPRIMIR_CADENA
    CALL CALCULAR_PUNTAJE
    CALL ACUMULAR_PUNTAJE           ; P2: sumar puntaje al jugador activo
    LEA  DX, MSG_PT_RONDA
    CALL IMPRIMIR_CADENA
...
FIN_RONDA:
    ...
    CALL CAMBIO_TURNO               ; P2: alterna TURNO y resetea datos
    JMP  RONDA
...
;====================================================================
; ============================================================
;              MODULOS - PERSONA 2 (Logica del juego)
; ============================================================
;====================================================================
...
;====================================================================
; ============================================================
;              MODULOS - PERSONA 3 (Archivos + Aleatoriedad)
; ============================================================
;====================================================================
...
; LEER_TECLA_DIRECTA: Lee sin eco, convierte minusculas a mayusculas
